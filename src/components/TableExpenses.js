...
class TableExpenses extends React.Component {
  render() {
    const { wallet, deleteItem, editingItem } = this.props;
    const { expenses } = wallet;
    return (
      (expenses.length === 0)
        ? <p className="no-expense">Nenhuma Despesa</p>
        : (
...
const mapDispatchToProps = (dispatch) => ({
  ...
  editingItem: (id) => dispatch(isEditingWallet(id)),
});

TableExpenses.propTypes = {
  wallet: PropTypes.shape.isRequired,
  deleteItem: PropTypes.func.isRequired,
  editingItem: PropTypes.func.isRequired,
};
